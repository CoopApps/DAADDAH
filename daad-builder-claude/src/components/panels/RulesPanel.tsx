...
const PROCESS_LABELS: Record<string, string> = {
  PRO0: "PRO 0 — Per-turn events (NPC follow, scenery checks)",
  PRO4: "PRO 4 — Auto events (timers, NPC schedules)",
  PRO5: "PRO 5 — Response table (player verb/noun commands)",
};

// Infrastructure tables generated by codegen — not shown in main dropdown
const INFRASTRUCTURE_TABLES: Record<string, string> = {
  PRO1: "Input loop (PARSE, turn counter)",
  PRO2: "Parse error handler",
  PRO3: "Post-description (LISTOBJ)",
  PRO6: "Initialization (runs once at game start)",
  PRO7: "Text window up",
  PRO8: "Text window down",
  PRO9: "Window position setup",
  PRO10: "Exits listing",
  PRO11: "Status line update",
  PRO12: "Turns counter display",
};
  ...
  return (
    <div className="panel-content" style={{ display: "flex", gap: 20, height: "calc(100vh - 180px)" }}>
      ...
      <div style={{ width: 290, display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {/* Process filter — user tables + custom */}
        <select
        ...
        >
          ...
          <optgroup label="Game Logic (put your rules here)">
            <option value="PRO5">PRO 5 — Response table (verb/noun commands)</option>
            <option value="PRO0">PRO 0 — Per-turn events (NPC follow, checks)</option>
            <option value="PRO4">PRO 4 — Auto events (timers, schedules)</option>
          </optgroup>
          <optgroup label="Infrastructure (auto-generated, advanced only)">
            <option value="PRO1">PRO 1 — Input loop</option>
            <option value="PRO2">PRO 2 — Parse error</option>
            <option value="PRO3">PRO 3 — Post-description</option>
          </optgroup>
          {/* Dynamic: show any custom process tables (PRO13+) used by rules */}
          {(() => {
            const custom = Array.from(new Set((game.rules || []).map(r => r.process)))
              .filter(p => !["PRO0","PRO1","PRO2","PRO3","PRO4","PRO5"].includes(p))
              .sort();
            return custom.length > 0 ? (
              <optgroup label="Custom tables (PRO 13-255)">
                {custom.map(p => <option key={p} value={p}>{p.replace("PRO", "PRO ")} — Custom</option>)}
              </optgroup>
            ) : null;
          })()}
        </select>
            ...
            <div className="card" style={{ marginBottom: 16, position: "sticky", top: 0, zIndex: 10 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto auto", gap: 12 }}>
                <div className="form-group">
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    ...
                    >
                      <optgroup label="Game Logic">
                        <option value="PRO5">PRO 5 — Response table</option>
                        <option value="PRO0">PRO 0 — Per-turn events</option>
                        <option value="PRO4">PRO 4 — Auto events</option>
                      </optgroup>
                      <optgroup label="Infrastructure (advanced)">
                        <option value="PRO1">PRO 1 — Input loop</option>
                        <option value="PRO2">PRO 2 — Parse error</option>
                        <option value="PRO3">PRO 3 — Post-description</option>
                      </optgroup>
                      ...
                      {(() => {
                        const custom = Array.from(new Set((game.rules || []).map(r => r.process)))
                          ...
                          .sort();
                        return custom.length > 0 ? (
                          <optgroup label="Custom (PRO 13-255)">
                            {custom.map(p => <option key={p} value={p}>{p.replace("PRO", "PRO ")}</option>)}
                          </optgroup>
                        ) : null;
                      })()}
                    </select>
              ...
              <div style={{ marginTop: 8, fontSize: 10, color: "var(--text-dim)" }}>
                {selectedRuleData.process === "PRO5" && (
                  <span>
                    <strong style={{ color: "var(--green-bright)" }}>Response Table:</strong> Your main game logic. Set verb + noun above. The player types a command and this rule fires if conditions match. End with DONE.
                  </span>
                )}
                {selectedRuleData.process === "PRO0" && (
                  <span>
                    <strong style={{ color: "var(--cyan-bright)" }}>Per-Turn Events:</strong> Runs every turn before the location description. Use <code>_ _</code> for unconditional events (NPC movement, state checks). Rules with verb/noun also match player commands.
                  </span>
                )}
                {selectedRuleData.process === "PRO4" && (
                  <span>
                    <strong style={{ color: "var(--amber-bright)" }}>Auto Events:</strong> Runs before each input prompt. Use for timers, NPC schedules, countdown fuses, turn-based events. Always <code>_ _</code>.
                  </span>
                )}
                {(selectedRuleData.process === "PRO1" || selectedRuleData.process === "PRO2" || selectedRuleData.process === "PRO3") && (
                  <span>
                    <strong style={{ color: "var(--text-dim)" }}>Infrastructure:</strong> Auto-generated by the compiler. Only add rules here if you know what you're doing. PRO 1-3 and PRO 6-12 handle input, errors, windows, and status bar.
                  </span>
                )}
                {!["PRO0","PRO1","PRO2","PRO3","PRO4","PRO5"].includes(selectedRuleData.process) && (
                  <span>
                    <strong style={{ color: "var(--magenta-bright)" }}>Custom Table:</strong> Called via PROCESS {selectedRuleData.process.replace("PRO","")} action from other rules. DAAD supports PRO 0-255 (256 tables max).
                  </span>
                )}
              </div>
            </div>

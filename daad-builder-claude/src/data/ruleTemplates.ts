...
export const RULE_TEMPLATES: RuleTemplate[] = [
  ...
  // ── Advanced Game Patterns (from Rabenstein/Carrados) ──────────────────

  {
    id: "hidden-compartment",
    name: "Hidden Compartment Puzzle",
    category: "Puzzles & Logic",
    description: "FEEL/SEARCH reveals hidden catch, OPEN reveals contents (Carrados drawer pattern)",
    difficulty: "advanced",
    tags: ["puzzle", "search", "container", "hidden"],
    rule: {
      name: "Feel Object - Find Hidden Catch",
      process: "PRO5",
      enabled: true,
      conditions: ["AT 0"],
      actions: ["MESSAGE 1", "SET 64", "CREATE 2", "DONE"],
    },
    instructions: "At location 0, FEEL reveals catch (flag 64 set, object 2 created). Add a second rule: OPEN + NOTZERO 64 → reveal contents",
  },
  {
    id: "multi-evidence-gate",
    name: "Multi-Evidence Win Condition",
    category: "Game State",
    description: "Require N pieces of evidence (flags) to trigger accusation/win. Carrados pattern.",
    difficulty: "advanced",
    tags: ["ending", "evidence", "gate", "detective"],
    rule: {
      name: "Accuse - Full Evidence",
      process: "PRO5",
      enabled: true,
      conditions: ["AT 0", "NOTZERO 64", "NOTZERO 65", "NOTZERO 66"],
      actions: ["MESSAGE 1", "MESSAGE 2", "END"],
    },
    instructions: "At location 0, all evidence flags (64-66) must be set. Add a fallback rule with same verb/noun but no flag checks → 'insufficient evidence' message + DONE",
  },
  {
    id: "npc-schedule",
    name: "NPC Schedule (Timed Movement)",
    category: "NPCs & Dialogue",
    description: "Move NPC between locations based on turn count. PRO4 auto-event.",
    difficulty: "advanced",
    tags: ["npc", "schedule", "movement", "timer"],
    rule: {
      name: "NPC Morning Schedule",
      process: "PRO4",
      enabled: true,
      conditions: ["LT 31 10"],
      actions: ["PLACE 0 1"],
    },
    instructions: "Object 0 (NPC) placed at location 1 when turns (flag 31) < 10. Add more rules for different time ranges: GT 31 10 + LT 31 20 → PLACE at location 2, etc.",
  },
  {
    id: "conversation-topic",
    name: "Conversation Topic (ASK NPC TOPIC)",
    category: "NPCs & Dialogue",
    description: "ASK [character] about [topic] with flag tracking for new/seen responses",
    difficulty: "intermediate",
    tags: ["npc", "dialogue", "ask", "conversation"],
    rule: {
      name: "Ask NPC About Topic",
      process: "PRO5",
      enabled: true,
      conditions: ["PRESENT 0", "ZERO 64"],
      actions: ["MESSAGE 1", "SET 64", "DONE"],
    },
    instructions: "When NPC (object 0) is present and topic not yet discussed (flag 64=0), show response and mark as discussed. Add second rule with NOTZERO 64 for repeat response.",
  },
  {
    id: "locked-door-key",
    name: "Locked Door + Key",
    category: "Puzzles & Logic",
    description: "Door blocks exit until player uses the correct key. Complete lock/unlock pattern.",
    difficulty: "intermediate",
    tags: ["puzzle", "door", "key", "lock"],
    rule: {
      name: "Unlock Door With Key",
      process: "PRO5",
      enabled: true,
      conditions: ["AT 0", "CARRIED 1", "ZERO 64"],
      actions: ["MESSAGE 2", "SET 64", "DONE"],
    },
    instructions: "At location 0, carrying key (object 1), door unlocked (flag 64 set). Add blocking rule: direction verb + AT 0 + ZERO 64 → 'The door is locked' + DONE",
  },
  {
    id: "timed-fuse",
    name: "Timed Fuse / Countdown",
    category: "Combat & Danger",
    description: "Countdown timer in PRO4 that triggers game over when reaching zero",
    difficulty: "advanced",
    tags: ["timer", "countdown", "danger", "fuse"],
    rule: {
      name: "Fuse Burns Down",
      process: "PRO4",
      enabled: true,
      conditions: ["NOTZERO 64", "GT 65 0"],
      actions: ["MINUS 65 1"],
    },
    instructions: "Flag 64 = fuse active, flag 65 = turns remaining. Add: NOTZERO 64 + ZERO 65 → explosion MESSAGE + END. Light fuse: SET 64 + LET 65 10 (10 turns)",
  },
  {
    id: "random-atmosphere",
    name: "Random Atmospheric Message",
    category: "Environment",
    description: "CHANCE-based atmospheric text that fires randomly each turn",
    difficulty: "beginner",
    tags: ["atmosphere", "random", "environment", "immersion"],
    rule: {
      name: "Random Sound",
      process: "PRO4",
      enabled: true,
      conditions: ["CHANCE 15", "AT 0"],
      actions: ["MESSAGE 1", "DONE"],
    },
    instructions: "15% chance each turn at location 0. Add multiple rules with different messages for variety. Use CHANCE 10-30 for good pacing.",
  },
  {
    id: "map-reveal",
    name: "First Visit / Revisit Text",
    category: "Environment",
    description: "Show different description on first visit vs repeat visits using a visited flag",
    difficulty: "intermediate",
    tags: ["location", "visit", "discovery", "map"],
    rule: {
      name: "First Visit - Discover Location",
      process: "PRO0",
      enabled: true,
      conditions: ["AT 0", "ZERO 64"],
      actions: ["MESSAGE 1", "SET 64"],
    },
    instructions: "Flag 64 tracks if location 0 has been visited. First visit shows discovery message (MESSAGE 1). The standard DESC shows on revisit. Use one flag per location.",
  },
  {
    id: "darkness-torch",
    name: "Darkness + Torch Puzzle",
    category: "Environment",
    description: "Dark room requires finding and lighting a torch. Complete light source pattern.",
    difficulty: "intermediate",
    tags: ["darkness", "light", "torch", "puzzle"],
    rule: {
      name: "Light Torch",
      process: "PRO5",
      enabled: true,
      conditions: ["CARRIED 0", "ZERO 64"],
      actions: ["MESSAGE 1", "SET 64", "DONE"],
    },
    instructions: "Object 0 = torch, flag 64 = lit. Mark the torch as object 0 (light source in OBJ). Mark target rooms as isDark. The engine auto-checks object 0 for light.",
  },
  {
    id: "companion-describe",
    name: "Companion Describes Room",
    category: "NPCs & Dialogue",
    description: "ASK companion to describe surroundings (Carrados/Carlyle pattern for blind detective)",
    difficulty: "intermediate",
    tags: ["npc", "companion", "describe", "carlyle"],
    rule: {
      name: "Ask Companion to Describe",
      process: "PRO5",
      enabled: true,
      conditions: ["PRESENT 0"],
      actions: ["MESSAGE 1", "DESC"],
    },
    instructions: "Object 0 = companion NPC. DESC shows current location description. Companion provides visual context the player can't see alone.",
  },
];
